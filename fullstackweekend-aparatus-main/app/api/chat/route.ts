...
export const POST = async (request: Request) => {
  // 1) Lê as mensagens que o useChat mandou do frontend
  const { messages } = await request.json();

  // 2) Chama o modelo de IA (Gemini) usando o AI SDK
  const result = streamText({
    // Usa o Gemini 1.5 Flash com a GOOGLE_GENERATIVE_AI_API_KEY do .env
    model: google("models/gemini-1.5-flash"),
    stopWhen: stepCountIs(10),

    // 3) Prompt de sistema: define o papel do assistente
    ...
    // 4) Converte mensagens do formato do useChat para o formato que o modelo espera
    messages: convertToModelMessages(messages),

    // 5) Ferramentas (tools) que o modelo pode usar pra buscar dados reais no banco
    tools: {
      // 1) Buscar barbearias
      ...
      // 2) Buscar horários disponíveis da barbearia
  ...
  // 6) Responde no formato que o DefaultChatTransport / useChat espera (stream)
